...
[EffectGroup("Classes/Warrior")]
[CreateAssetMenu(fileName = "New Turtle", menuName = "Status Effects/Turtle", order = 1)]
public class Turtle : Effect
{

    public int maxResist;
    public int resistPerTurn;
    ...
    {
        return new EffectDisplayInfo(0, "Lasts until you move");
    ...
    {
        return $"You are currently gaining {resist} armor from turtling.";
    }
    ...
    {
        if (connectedTo.monster.location != location)
        {
            resist = 0;
            location = connectedTo.monster.location;
        }
        else
        {
            resist += resistPerTurn;
            if (resist > maxResist)
            {
                resist = maxResist;
            }
