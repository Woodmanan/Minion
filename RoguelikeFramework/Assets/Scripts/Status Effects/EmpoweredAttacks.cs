...
[CreateAssetMenu(fileName = "New EmpoweredAttacks", menuName = "Status Effects/EmpoweredAttacks", order = 1)]
public class EmpoweredAttacks : Effect
{
    ...
    {
        if (result == AttackResult.HIT && action.caller.resources.mana > 0)
        {
            action.caller.resources.mana -= 1;
            action.target.Damage(action.caller, 2, DamageType.FIRE, DamageSource.EFFECT, "Your mana burns the {name}!");
        }
